...
import { applyTheme, loadSavedTheme, extractColorsFromImage, saveTheme, DEFAULT_THEME } from '../../lib/colorExtractor';
...
export default function AppLayout() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      extractColorsFromImage(company.logo_url)
        .then(colors => { applyTheme(colors); saveTheme(colors); })
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
        <div className="sidebar-logo">
          {company?.logo_url
            ? <img src={company.logo_url} alt="" className="logo-img" />
...
const shellStyles = `
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .logo-icon{width:30px;height:30px;border-radius:8px;background:var(--c-primary);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff}
  ...
  .nav-item.active{background:#1a1a2e;color:var(--c-primary-text)}
  ...
  .user-avatar{width:28px;height:28px;border-radius:7px;background:var(--c-primary)22;border:1px solid var(--c-primary)44;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:var(--c-primary-text);flex-shrink:0}
